...
import TileNode, {
    getTileById,
    TileType,
} from "../../../contexts/gameManager/TileNode";
...
export const warpGame = (
    root: TileNode,
    warp: (innerGame: TileNode[][]) => TileNode[][],
    ...
    warpRoot.innerGame = warpRoot.innerGame.map((row) =>
        row.map((tile) => warpGame(tile, warp, depth - 1)),
    );
...
export const forWarpAll = (
    root: TileNode,
    warp: (x: number, y: number, tile: TileNode) => TileNode,
    depth: number = Infinity,
    id: string | null = null,
): TileNode => {
    return warpGame(
...
export const warpTileTypeGame = (
    root: TileType,
    warp: (innerGame: TileType[]) => TileType[],
    depth: number = Infinity,
    id: string | null = null,
): TileType => {
    if (!root.innerGame) return root;
    ...
    warpRoot.innerGame = warpRoot.innerGame.map((tile) =>
        warpTileTypeGame(tile, warp, depth - 1),
    );
    warpRoot.innerGame = warp(warpRoot.innerGame);
...
export const forWarpAllTileType = (
    root: TileType,
    warp: (index: number, tile: TileType) => TileType,
    depth: number = Infinity,
    id: string | null = null,
): TileType => {
    return warpTileTypeGame(
        root,
        (innerGame) => {
...
export const basicTileType = (
    warpGames?: (innerGame: TileType[]) => TileType[],
): TileType => {
