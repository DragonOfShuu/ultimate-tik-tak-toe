...
import parseChanges, {
    parseChange,
} from "../../libs/valueManagement/ValueManager";
...
test("Parse change", () => {
    const changes = parseChange(
        ...
        "temperature",
        100,
        exampleRuleList.temperature,
    );
    expect.soft(changes).toEqual({ temperature: 100 });
    const change1 = parseChange(
        exampleData,
        "temperature",
        101,
        exampleRuleList.temperature,
...
test("Parse changes", () => {
    let expectedChanges: Partial<ExampleDataType> = {
        lowerTemp: 18,
        temperature: 20,
    };
    ...
    expect
        .soft(changes)
        .toEqual({ ...exampleData, temperature: 18, lowerTemp: 18 });
});
...
test("MinMaxRule getting other values", () => {
    ...
    expect
        .soft(exampleRuleList.lowerTemp.test(exampleData, -3894753))
        .toBe(true);
